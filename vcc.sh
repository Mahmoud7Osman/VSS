...
help(){
  ...
  printf "$yellow          ish        : $white Inject payload in shell  script  $green      (  Shell  Trojan   ) \n"
  printf "$yellow          ipy        : $white Inject payload in python script  $green      (  Python Trojan   ) \n"
...
while [ 1 ];do
 ...
 if [ -f /bin/$(echo $vcmd | head -n1 | awk '{print $1;}') ];then
    sound "s"
    printf "$red[-]$white Bash Command blocked inside VSS\n"
 elif [ '$vcmd' == '' ];then
    printf ''; break;
